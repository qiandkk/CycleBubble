...
本仓库已切换为 **根目录发布**（即从仓库 Settings → Pages → Source 选择 `/ (root)` / 对应分支根）。
`docs/` 历史副本已删除，无需手动同步脚本和样式。

若此前 Source 设置为 `/docs`，请在仓库 Settings → Pages 中改为根目录。

## 数据库迁移

后端从 Phase 2 起引入 [alembic](https://alembic.sqlalchemy.org/) 做 schema 版本化管理。
