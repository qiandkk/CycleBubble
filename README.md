...
```
.
├── README.md                ← 本文件：项目入口（唯一根 markdown）
│
├── frontend/                ← GH Pages 发布源（5 屏 SPA）
│   ├── index.html             GH Pages 入口
...
本仓库的 GH Pages 源是 **`frontend/` 子目录**（在 repo Settings → Pages 选 Branch `master`、Folder `/frontend`）。

这样前端代码独立成 `frontend/` 包，跟后端 `backend/` 完全隔离，根目录只保留文档与配置。

所有资源路径都是 `./xxx` 相对引用，无论怎么改路径都自动跟着 index.html 的位置解析——少点硬编码、少点坑。
